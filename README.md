...
├── main.js                # i18n toggle + lightbox logic
├── Images/                # Full-resolution behind-the-scenes photography
│   └── thumbs/            # Generated, compressed thumbnails for the gallery grid
└── .github/workflows/
...
To regenerate gallery thumbnails after adding new photos to `Images/`:

```bash
for f in Images/*.jpeg; do
  convert "$f" -auto-orient -resize '1000x1000>' -strip -quality 78 "Images/thumbs/$(basename "$f")"
done
```
